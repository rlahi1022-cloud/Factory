// ============================================================================
// ClientConfig.cpp — config.json 로더 (MFC 클라이언트 전용)
// ============================================================================
// 책임:
//   프로젝트 루트의 config/config.json 중 `client` + `network` 섹션을 읽어
//   서버 IP/포트/수신버퍼 크기 등 접속 파라미터를 정적 멤버로 노출한다.
//
// 경로 탐색 순서 (우선순위):
//   1) EXE 위치 기준 상대경로 (../../../config/config.json)
//   2) 작업 디렉터리 기준
//   3) 실패 시 ClientProtocol.h 의 DEFAULT_* 상수 사용
//
// 싱글톤 + 정적 멤버:
//   여러 페이지/다이얼로그에서 ClientConfig::GetServerIp() 로 접근.
//   Load() 는 앱 시작 시 1회만 호출 — 이후 s_loaded 플래그로 중복 로드 차단.
// ============================================================================
#include "pch.h"
