...
def create_data_yaml(data_dir: str, output_path: str) -> str:
    """YOLO 학습용 data.yaml 파일을 자동 생성하는 함수.

    용도:
      YOLO 학습에는 반드시 data.yaml 설정 파일이 필요하다.
      이 파일에는 학습/검증 이미지 경로와 클래스 이름이 정의되어 있다.
      수동으로 만들기 번거로우므로, 이 함수가 자동으로 생성해준다.

    data_dir 폴더 구조 (YOLO 표준 형식):
      data_dir/
        images/
          train/   (학습용 이미지 파일들)
          val/     (검증용 이미지 파일들)
        labels/
          train/   (학습용 라벨 파일들, YOLO 형식: 클래스 x중심 y중심 너비 높이)
          val/     (검증용 라벨 파일들)
    ...
    # YAML 형식의 설정 내용을 문자열로 만든다.
    # f-string을 사용하여 data_dir의 절대 경로를 삽입한다.
...
nc: 3
"""
    # path: 데이터 루트 경로 (절대 경로로 변환). resolve()는 상대경로를 절대경로로 바꿔준다.
    # train: 학습 이미지 폴더 (path 기준 상대 경로)
    # val: 검증 이미지 폴더
    # names: 클래스 번호와 이름 매핑. 0번=cap, 1번=label, 2번=liquid_level
    # nc: Number of Classes (클래스 수) = 3개

    # 출력 경로를 Path 객체로 변환한다.
    output_path = Path(output_path)

    # 상위 폴더가 없으면 자동 생성한다.
    output_path.parent.mkdir(parents=True, exist_ok=True)

    # YAML 내용을 파일로 저장한다. UTF-8 인코딩을 사용한다.
    output_path.write_text(yaml_content, encoding="utf-8")

    # 생성 완료 로그를 남긴다.
    logger.info("Created data.yaml: %s", output_path)

    # 생성된 파일의 경로를 문자열로 반환한다.
