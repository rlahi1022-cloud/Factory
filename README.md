# Factory — Event-Driven Smart Factory Inspection Platform

> CNN 기반 페트병 공정 모니터링 시스템
...
전체 시스템 구조를 정하기 전, 다음 4가지 결정을 먼저 확정했다.
각 결정은 단순한 역할 분담이 아니라 시스템 통합과 안정성을 고려한 구조적 판단이었다.

### 1. Main Server 중심 (Hub-and-Spoke)

* **결정**: 모든 통신이 Main Server를 거치는 중앙 집중 구조
* **고려한 대안**: AI ↔ Client 직접 통신 / 각 서버 P2P 분산 구조
* **선택 이유**:
  - 데이터 흐름이 분산되면 상태 관리·장애 추적이 어려워짐
  - DB 저장·로그·통계 처리 위치가 불명확해짐
  - 단순 기능 구현이 아니라 공정 전체를 관리하는 시스템이기 때문에, 중앙에서 모든 흐름을 통제할 필요가 있다고 판단
* **결과**: 모든 결과가 Main Server를 통해 수집되고, DB 저장·후처리·클라이언트 push가 단일 경로로 통제됨

### 2. AI 학습 / 추론 서버 분리

* **결정**: 학습 서버와 추론 서버를 독립 프로세스로 분리
* **고려한 대안**: 단일 AI 서버에서 학습·추론을 통합 처리
* **선택 이유**:
  - 학습은 GPU 자원을 지속적으로 점유 → 실시간 처리 지연 발생
  - 추론은 실시간성이 중요한데, 학습과 함께 두면 충돌 가능
  - 구조적으로 역할이 다른데 하나로 묶이면 확장성이 떨어짐
* **결과**: 학습은 비동기·배치 중심, 추론은 실시간 판정에 집중. `server_type` 기반 매칭으로 학습서버를 다른 PC로 옮기는 작업이 config 한 줄 수정으로 가능

### 3. Client = Event 수신 Viewer

* **결정**: 서버 push 이벤트를 받아 UI만 갱신하는 구조
* **고려한 대안**: Client polling 방식 (주기적 요청)
* **선택 이유**:
  - 공정 기반 시스템은 이벤트가 발생했을 때만 데이터가 의미 있음
  - polling은 불필요한 네트워크 트래픽을 발생시키고, 실시간 시스템에서 반응 지연을 만듦
  - Client는 데이터를 요청하는 주체가 아니라 서버 상태를 반영하는 Viewer 역할로 정의하는 것이 적합
* **결과**: NG 발생 즉시 클라이언트 broadcast, 1초 타이머로 소켓 상태 실시간 동기화

### 4. OK / NG 분리 패킷 구조

* **결정**: OK는 JSON 메타데이터만, NG는 JSON + 이미지 전송
* **고려한 대안**: 모든 검사 결과에 이미지 포함 전송
* **선택 이유**:
  - 라인 속도 기준으로 모든 결과에 이미지를 포함하면 네트워크 부하가 크게 증가
  - 정상 데이터는 가볍게 처리하고 문제 있는 데이터만 상세 기록을 남기는 구조가 적합
* **결과**: OK는 fire-and-forget(`INSPECT_META 1006`), NG는 ACK 기반 재전송(`STATION_NG 1000/1002`)으로 신뢰성 확보
